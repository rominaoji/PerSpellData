...
with open('./results/confusion_transposition.csv', 'a', newline='', encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["word1", "word2", "type", "edit_distance", "no"])
    for word_length in range(0, max + 1):
        print("Transposition", word_length)
        if word_length > 0:
            for word1 in list_variable_length[word_length]:
                for word2 in list_variable_length[word_length]:
                    if (edit_distance(word1, word2) == 2):
                        if (word1[0] == word2[0]):
                            join_word = zip(word1, word2)
                            for char_word1 in range(0, len(word1) - 1):
                                if word1[char_word1] != word2[char_word1] and word1[char_word1] != "‌" and word2[
                                    char_word1] != "\u200c" and word1[char_word1 + 1] != "\u200c" and word2[
                                    char_word1 + 1] != "\u200c":
                                    if (word1[char_word1] == word2[char_word1 + 1] and word2[char_word1] == word1[
                                        char_word1 + 1] and (set(word1) == set(word2))):
                                        writer.writerow([word1, word2, df1.loc[word1[char_word1], word2[char_word1]],
                                                         "Transposition", word_length + 1])
