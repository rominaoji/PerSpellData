...
with open('results/confusion_set_substitution.csv', 'a', newline='', encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["word1", "word2", "type", "edit_distance", "no"])
    for word_length in range(0, max + 1):
        print("substitution", word_length)
                ...
                if (edit_distance(word1, word2) == 1):
                    ...
                    for char_word1, char_word2 in join_word:
                        if char_word1 != char_word2:
                            if char_word1 != "\u200c" and char_word1 != " " and char_word2 != "\u200c" and char_word2 != " ":
                                writer.writerow(
                                    [word1, word2, df4.loc[[char_word1], [char_word2]].values[0][0], "Substitution",
                                     word_length + 1])
                            elif char_word1 == "\u200c" or char_word2 == "\u200c":
                                writer.writerow([word1, word2, 2.6, "substitution", word_length + 1])
                            elif char_word1 == " " or char_word2 == " ":
                                writer.writerow([word1, word2, 2.9, "substitution", word_length + 1])
