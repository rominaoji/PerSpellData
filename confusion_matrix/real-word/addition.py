...
with open('results/confusion_addition.csv', 'a', newline='', encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["word1", "word2", "type", "edit_distance", "no"])
    for word_length in range(0, max + 1):
        print("Addition", word_length)
        if word_length != max:
                ...
                for word2 in list_variable_length[word_length + 1]:
                    if (edit_distance(word1, word2) == 1):
                        if (word1[0] == word2[0]):
                            ...
                            for char_word1 in range(0, minlen):
                                if (word2[char_word1] == "\u200c" or word2[char_word1 - 1] == "\u200c"):
                                    writer.writerow([word1, word2, 2.6, "Addition", word_length + 1])
                                else:
                                    writer.writerow(
                                        [word1, word2, df2.loc[word2[char_word1 - 1], word2[char_word1]], "Addition",
                                         word_length + 1])
